...
export class LemonSqueezyPaymentService implements PaymentProviderPort {
  ...
  async createCheckout(
    ...
    if (!variantId) {
      throw new Error(
        `No Lemon Squeezy variant configured for plan: ${params.plan}`,
      );
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Lemon Squeezy checkout failed (${response.status}): ${errorBody}`,
      );
    }
  ...
  async cancelSubscription(externalSubscriptionId: string): Promise<void> {
    const response = await fetch(
      `${LS_API_BASE}/subscriptions/${externalSubscriptionId}`,
      {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${this.apiKey}`,
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      throw new Error(
        `Lemon Squeezy cancel failed (${response.status}): ${errorBody}`,
      );
    }
  }
  ...
  async getCustomerPortalUrl(
    ...
    try {
      const response = await fetch(
        `${LS_API_BASE}/customers/${externalCustomerId}`,
        {
          headers: {
            Authorization: `Bearer ${this.apiKey}`,
            Accept: 'application/vnd.api+json',
