...
import { DeliveryAccessTokenModel, DeliveryAccessTokenDocument } from '../schemas/delivery-access-token.schema.js';
...
export class MongoDeliveryAccessTokenRepository implements DeliveryAccessTokenRepository {
  constructor(@InjectModel(DeliveryAccessTokenModel.name) private readonly model: Model<DeliveryAccessTokenDocument>) {}

  async create(data: Omit<DeliveryAccessToken, 'id' | 'createdAt' | 'revokedAt'>): Promise<DeliveryAccessToken> {
  ...
  async findByRestaurantId(restaurantId: string): Promise<DeliveryAccessToken[]> {
