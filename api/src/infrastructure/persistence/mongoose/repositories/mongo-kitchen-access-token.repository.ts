...
import {
  ...
  KitchenAccessTokenDocument,
} from '../schemas/kitchen-access-token.schema.js';
...
export class MongoKitchenAccessTokenRepository implements KitchenAccessTokenRepository {
  constructor(
    @InjectModel(KitchenAccessTokenModel.name)
    private readonly model: Model<KitchenAccessTokenDocument>,
  ) {}

  async create(
    data: Omit<KitchenAccessToken, 'id' | 'createdAt' | 'revokedAt'>,
