...
export class KitchenController {
  constructor(
    @Inject('CreateKitchenTokenUseCase')
    private readonly createToken: CreateKitchenTokenUseCase,
    @Inject('ValidateKitchenTokenUseCase')
    private readonly validateToken: ValidateKitchenTokenUseCase,
    @Inject('ListKitchenTokensUseCase')
    private readonly listTokens: ListKitchenTokensUseCase,
    @Inject('RevokeKitchenTokenUseCase')
    private readonly revokeToken: RevokeKitchenTokenUseCase,
  ...
  async getOrderDetail(@Req() req: Request, @Param('id') id: string) {
    ...
    const orderResult = await this.getOrder.execute(
      id,
  ...
  async updateOrderStatus(
    ...
    const result = await this.updateStatus.execute(
      id,
      tokenResult.value.restaurantId,
