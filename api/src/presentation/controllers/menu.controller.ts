...
export class MenuController {
  constructor(
    ...
    private readonly toggleAvailability: ToggleMenuItemAvailabilityUseCase,
    @Inject('ReorderMenuItemsUseCase')
    private readonly reorderItems: ReorderMenuItemsUseCase,
    @Inject('CreateMenuItemVariantUseCase')
    private readonly createVariant: CreateMenuItemVariantUseCase,
    @Inject('UpdateMenuItemVariantUseCase')
    private readonly updateVariant: UpdateMenuItemVariantUseCase,
    @Inject('DeleteMenuItemVariantUseCase')
    private readonly deleteVariant: DeleteMenuItemVariantUseCase,
    @Inject('CreateMenuItemOptionUseCase')
    private readonly createOption: CreateMenuItemOptionUseCase,
    @Inject('UpdateMenuItemOptionUseCase')
    private readonly updateOption: UpdateMenuItemOptionUseCase,
    @Inject('DeleteMenuItemOptionUseCase')
    private readonly deleteOption: DeleteMenuItemOptionUseCase,
  ) {}
  ...
  async reorderCats(
    @CurrentUser() user: RequestUser,
    @Body(new ZodValidationPipe(ReorderRequestSchema)) body: ReorderRequestDto,
  ) {
  ...
  async createIt(
    @CurrentUser() user: RequestUser,
    @Body(new ZodValidationPipe(CreateItemRequestSchema))
  ...
  async updateIt(
    @CurrentUser() user: RequestUser,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateItemRequestSchema))
  ...
  async reorderIts(
    ...
    const result = await this.reorderItems.execute(
      user.restaurantId,
      body.items,
    );
    if (!result.ok) throw new ForbiddenException(result.error.message);
  ...
  async createVar(
    ...
    @Body(new ZodValidationPipe(CreateVariantRequestSchema))
    body: CreateVariantRequestDto,
  ) {
    const result = await this.createVariant.execute(user.restaurantId, {
      ...body,
      itemId,
    });
    if (!result.ok)
  ...
  async updateVar(
    @CurrentUser() user: RequestUser,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateVariantRequestSchema))
  ...
  async createOpt(
    @CurrentUser() user: RequestUser,
    @Param('itemId') itemId: string,
    @Body(new ZodValidationPipe(CreateOptionRequestSchema))
    body: CreateOptionRequestDto,
  ) {
    const result = await this.createOption.execute(user.restaurantId, {
      ...body,
      itemId,
    });
    if (!result.ok)
  ...
  async updateOpt(
    @CurrentUser() user: RequestUser,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateOptionRequestSchema))
