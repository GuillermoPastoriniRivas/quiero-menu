...
export class DeliveryController {
  constructor(
    @Inject('CreateDeliveryTokenUseCase')
    private readonly createToken: CreateDeliveryTokenUseCase,
    @Inject('ValidateDeliveryTokenUseCase')
    private readonly validateToken: ValidateDeliveryTokenUseCase,
    @Inject('ListDeliveryTokensUseCase')
    private readonly listTokens: ListDeliveryTokensUseCase,
    @Inject('RevokeDeliveryTokenUseCase')
    private readonly revokeToken: RevokeDeliveryTokenUseCase,
  ...
  private async validateDeliveryToken(req: Request): Promise<string> {
    ...
    if (!deliveryToken)
      throw new UnauthorizedException('Missing delivery token');
  ...
  async updateOrderStatus(
    ...
    if (!expectedNext || expectedNext !== status) {
      throw new BadRequestException(
        `Delivery can only transition from ${order.status} to ${expectedNext ?? 'nothing'}.`,
      );
    }

    const result = await this.updateStatus.execute(id, restaurantId, status);
