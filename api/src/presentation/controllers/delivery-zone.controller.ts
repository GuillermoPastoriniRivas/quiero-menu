...
} from '../request-dtos/delivery-zone.dto.js';
...
export class DeliveryZoneController {
  constructor(
    @Inject('CreateDeliveryZoneUseCase')
    private readonly createZone: CreateDeliveryZoneUseCase,
    @Inject('ListDeliveryZonesUseCase')
    private readonly listZones: ListDeliveryZonesUseCase,
    @Inject('UpdateDeliveryZoneUseCase')
    private readonly updateZone: UpdateDeliveryZoneUseCase,
    @Inject('DeleteDeliveryZoneUseCase')
    private readonly deleteZone: DeleteDeliveryZoneUseCase,
  ) {}
  ...
  async create(
    ...
    @Body(new ZodValidationPipe(CreateDeliveryZoneRequestSchema))
    body: CreateDeliveryZoneRequestDto,
  ) {
    const result = await this.createZone.execute({
      ...body,
  ...
  async update(
    @CurrentUser() user: RequestUser,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(UpdateDeliveryZoneRequestSchema))
    body: UpdateDeliveryZoneRequestDto,
  ) {
